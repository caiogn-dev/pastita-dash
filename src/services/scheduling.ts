...
export const scheduledMessagesService = {
  list: async (params: Record<string, string> = {}): Promise<PaginatedResponse<ScheduledMessage>> => {
    const response = await api.get(`${BASE_PATH}/scheduled-messages/`, { params });
    return response.data;
  },

  get: async (id: string): Promise<ScheduledMessage> => {
    const response = await api.get(`${BASE_PATH}/scheduled-messages/${id}/`);
    return response.data;
  },

  create: async (data: CreateScheduledMessage): Promise<ScheduledMessage> => {
    const response = await api.post(`${BASE_PATH}/scheduled-messages/`, data);
    return response.data;
  },

  update: async (id: string, data: Partial<CreateScheduledMessage>): Promise<ScheduledMessage> => {
    const response = await api.patch(`${BASE_PATH}/scheduled-messages/${id}/`, data);
    return response.data;
  },

  delete: async (id: string): Promise<void> => {
    await api.delete(`${BASE_PATH}/scheduled-messages/${id}/`);
  },

  cancel: async (id: string): Promise<{ success: boolean; message: string }> => {
    const response = await api.post(`${BASE_PATH}/scheduled-messages/${id}/cancel/`);
    return response.data;
  },

  reschedule: async (id: string, scheduledAt: string): Promise<ScheduledMessage> => {
    const response = await api.post(`${BASE_PATH}/scheduled-messages/${id}/reschedule/`, {
  ...
  getStats: async (accountId?: string): Promise<ScheduledMessageStats> => {
    ...
    const response = await api.get(`${BASE_PATH}/scheduled-messages/stats/`, { params });
    return response.data;
  },
};
...
export const reportSchedulesService = {
  list: async (params: Record<string, string> = {}): Promise<PaginatedResponse<ReportSchedule>> => {
    const response = await api.get(`${BASE_PATH}/report-schedules/`, { params });
    return response.data;
  },

  get: async (id: string): Promise<ReportSchedule> => {
    const response = await api.get(`${BASE_PATH}/report-schedules/${id}/`);
    return response.data;
  },

  create: async (data: CreateReportSchedule): Promise<ReportSchedule> => {
    const response = await api.post(`${BASE_PATH}/report-schedules/`, data);
    return response.data;
  },

  update: async (id: string, data: Partial<CreateReportSchedule>): Promise<ReportSchedule> => {
    const response = await api.patch(`${BASE_PATH}/report-schedules/${id}/`, data);
    return response.data;
  },

  delete: async (id: string): Promise<void> => {
    await api.delete(`${BASE_PATH}/report-schedules/${id}/`);
  },

  runNow: async (id: string): Promise<{ success: boolean; message: string; task_id: string }> => {
    const response = await api.post(`${BASE_PATH}/report-schedules/${id}/run_now/`);
    return response.data;
  },

  pause: async (id: string): Promise<{ success: boolean; message: string }> => {
    const response = await api.post(`${BASE_PATH}/report-schedules/${id}/pause/`);
    return response.data;
  },

  resume: async (id: string): Promise<{ success: boolean; message: string }> => {
    const response = await api.post(`${BASE_PATH}/report-schedules/${id}/resume/`);
    return response.data;
  },
};
...
export const generatedReportsService = {
  ...
  get: async (id: string): Promise<GeneratedReport> => {
    const response = await api.get(`${BASE_PATH}/reports/${id}/`);
    return response.data;
  },

  generate: async (data: GenerateReportRequest): Promise<{ success: boolean; message: string; task_id: string }> => {
    const response = await api.post(`${BASE_PATH}/reports/generate/`, data);
    return response.data;
  },

  download: async (id: string): Promise<Blob> => {
    const response = await api.get(`${BASE_PATH}/reports/${id}/download/`, {
      responseType: 'blob',
    });
    return response.data;
  },
  ...
  resendEmail: async (id: string, recipients?: string[]): Promise<{ success: boolean; message: string }> => {
    const response = await api.post(`${BASE_PATH}/reports/${id}/resend_email/`, {
      recipients,
    });
    return response.data;
  },
