...
export const UnifiedOrchestratorTest: React.FC = () => {
  ...
  const getSourceIcon = (source?: string) => {
    switch (source) {
      ...
      default:
        return <ServerIcon className="w-4 h-4 text-fg-muted-token" />;
    }
  };
  ...
  return (
    <div className="flex flex-row max-lg:flex-col gap-6">
      {/* Main Chat Area */}
      <div className="flex-1 flex flex-col h-[700px] bg-surface rounded-xl border border-border-token overflow-hidden">
        {/* Header */}
        ...
        {/* Phone Input */}
        <div className="px-4 py-2 bg-surface-2 border-b border-border-token flex items-center gap-2">
          <span className="text-sm text-fg-muted-token">Telefone:</span>
          <input
            type="text"
            value={phoneNumber}
            onChange={(e) => setPhoneNumber(e.target.value)}
            className="flex-1 px-3 py-1 text-sm bg-surface-2 border border-border-token rounded-lg"
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {['Oi', 'Cardápio', 'Ver carrinho', 'Status do pedido'].map((msg) => (
                  <button
                    ...
                    className="px-4 py-2 bg-surface-2 hover:bg-brand-soft rounded-full text-sm text-fg-token transition-colors"
                  >
                    {msg}
                  </button>
                ))}
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {message.buttons.map((btn) => (
                        <button
                          ...
                          className="px-3 py-1.5 bg-surface-2 border border-border-token rounded-lg text-sm text-fg-token hover:bg-brand-soft transition-colors"
                        >
                          {btn.title}
                        </button>
                      ))}
            ...
            <div className="flex gap-3 justify-start">
              ...
              <div className="bg-surface-2 rounded-2xl rounded-bl-md px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-fg-muted-token ml-2">
      ...
      <div className="w-full lg:w-80 space-y-4">
        {/* Flow Steps */}
        <div className="bg-surface rounded-xl border border-border-token p-4">
          <h3 className="font-medium text-fg-token mb-4 flex items-center gap-2">
          ...
          <div className="space-y-2">
            {FLOW_STEPS.map((step) => (
              <div
                key={step.id}
                className="p-3 rounded-lg bg-surface-2 hover:bg-brand-soft transition-colors"
              >
                <div className="flex items-center gap-2 mb-1">
                  <step.icon className="w-4 h-4 text-fg-muted-token" />
                  <span className="font-medium text-sm text-fg-token">
                    {step.title}
                  </span>
                </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {step.messages.map((msg) => (
                    <button
                      key={msg}
                      onClick={() => handleQuickTest(msg)}
                      className="px-2 py-1 bg-surface-2 border border-border-token rounded text-xs text-fg-muted-token hover:bg-brand-soft transition-colors"
                    >
                      {msg}
                    </button>
                  ))}
        ...
        {/* Sources Info */}
        <div className="bg-surface rounded-xl border border-border-token p-4">
          <h3 className="font-medium text-fg-token mb-4">
            Fontes de Resposta
          </h3>
          <div className="space-y-2">
            <div className="flex items-center gap-2 p-2 rounded-lg bg-purple-50 dark:bg-purple-900/20">
              ...
              <div>
                <span className="text-sm font-medium text-fg-token">Jasper Template</span>
                <p className="text-xs text-fg-muted-token">Templates profissionais com botões</p>
              </div>
            </div>
            <div className="flex items-center gap-2 p-2 rounded-lg bg-blue-50 dark:bg-blue-900/20">
              <DocumentTextIcon className="w-4 h-4 text-blue-500" />
              <div>
                <span className="text-sm font-medium text-fg-token">AutoMessage</span>
                <p className="text-xs text-fg-muted-token">Templates configuráveis do banco</p>
              </div>
            </div>
            <div className="flex items-center gap-2 p-2 rounded-lg bg-yellow-50 dark:bg-yellow-900/20">
              <BoltIcon className="w-4 h-4 text-yellow-500" />
              <div>
                <span className="text-sm font-medium text-fg-token">Handler</span>
                <p className="text-xs text-fg-muted-token">Lógica específica por intent</p>
              </div>
            </div>
            <div className="flex items-center gap-2 p-2 rounded-lg bg-green-50 dark:bg-green-900/20">
              ...
              <div>
                <span className="text-sm font-medium text-fg-token">LLM Direto</span>
                <p className="text-xs text-fg-muted-token">Resposta gerada pelo modelo</p>
              </div>
            </div>
          </div>
        </div>
