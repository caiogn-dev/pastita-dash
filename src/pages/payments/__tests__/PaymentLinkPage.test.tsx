...
describe('PaymentLinkPage — link de pagamento avulso (F5)', () => {
  it('gera link de pagamento avulso e exibe URL e botão para abrir', async () => {
  ...
  it('usa init_point quando payment_url não vem na resposta', async () => {
    mockCreatePaymentLink.mockResolvedValue({
      payment: { init_point: 'https://mp/init', amount: 15 },
      store_payment: { id: 'sp2', order: null },
    });

    render(<PaymentLinkPage />);
    fireEvent.change(screen.getByLabelText('Valor (R$)'), { target: { value: '15' } });
    fireEvent.click(screen.getByRole('button', { name: /Gerar link de pagamento/i }));
