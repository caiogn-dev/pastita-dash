    ...
    <div className="p-6">
      ...
      {stores.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <h3 className="text-lg font-medium text-fg-token dark:text-[var(--dark-text-primary,#FAF9F7)] mb-2">Nenhuma loja cadastrada</h3>
          <p className="text-fg-muted-token mb-4">Crie sua primeira loja para começar a vender</p>
          <Button onClick={() => setShowCreateModal(true)}>
            <Plus className="w-4 h-4 mr-2" />
            Criar Loja
          </Button>
            ...
            return (
              <Card key={store.id} className="overflow-hidden">
                    ...
                    <div className="flex items-center gap-3">
                      {store.logo_url ? (
                        ...
                        <div className="w-12 h-12 rounded-lg bg-surface-2 flex items-center justify-center">
                          <Store className="w-6 h-6 text-fg-muted-token" />
                        </div>
                      )}
                      <div>
                        <h3 className="font-semibold text-fg-token dark:text-[var(--dark-text-primary,#FAF9F7)]">{store.name}</h3>
                        <p className="text-sm text-fg-muted-token">{getStoreTypeLabel(store.store_type)}</p>
                      </div>
                    </div>
                ...
                <div className="p-4 grid grid-cols-3 gap-4 text-center border-b">
                  <div>
                    <p className="text-2xl font-bold text-fg-token dark:text-[var(--dark-text-primary,#FAF9F7)]">{store.products_count}</p>
                    <p className="text-xs text-fg-muted-token">Produtos</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-fg-token dark:text-[var(--dark-text-primary,#FAF9F7)]">{store.orders_count}</p>
                    <p className="text-xs text-fg-muted-token">Pedidos</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-fg-token dark:text-[var(--dark-text-primary,#FAF9F7)]">{store.integrations_count}</p>
                    <p className="text-xs text-fg-muted-token">Integrações</p>
                  </div>
                </div>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Nova Loja" size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-fg-token dark:text-[var(--dark-text-primary,#FAF9F7)] mb-1">Tipo de Loja</label>
          <select
        ...
        <div>
          <label className="block text-sm font-medium text-fg-token dark:text-[var(--dark-text-primary,#FAF9F7)] mb-1">Descrição</label>
          <textarea
