  ...
  return (
    <div className="grid grid-cols-3 max-lg:grid-cols-1 gap-6">
      ...
      <SectionCard title="Por pagamento" loading={q.isLoading}>
        <RankBarList
          items={(d?.by_payment_method ?? []).map((r) => ({ label: String(r.payment_method), value: Number(r.revenue) }))}
          valueFormat={formatBRL}
        />
      </SectionCard>
      <SectionCard title="Entrega × retirada" loading={q.isLoading}>
        <RankBarList
          items={(d?.by_delivery_method ?? []).map((r) => ({ label: String(r.delivery_method), value: Number(r.revenue) }))}
          valueFormat={formatBRL}
        />
      </SectionCard>
    </div>
