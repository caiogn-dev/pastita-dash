...
// Helper to check if store has valid coordinates
const hasValidCoordinates = (store: StoreLocation | null): boolean => {
  if (!store) return false;
  const lat = typeof store.latitude === 'string' ? Number.parseFloat(store.latitude) : store.latitude;
  const lng = typeof store.longitude === 'string' ? Number.parseFloat(store.longitude) : store.longitude;
...
export const DeliveryZonesPage: React.FC = () => {
  ...
  // Get store location for map display
  const storeMapLocation = useMemo(() => {
    if (!storeLocation || !hasValidCoordinates(storeLocation)) return null;
    const lat = typeof storeLocation.latitude === 'string' 
      ? Number.parseFloat(storeLocation.latitude) 
      : storeLocation.latitude;
    const lng = typeof storeLocation.longitude === 'string' 
      ? Number.parseFloat(storeLocation.longitude) 
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500">Calcule frete por quilometragem e gerencie faixas de preço</p>
        </div>
          ...
          <div className="mt-6 space-y-4">
            ...
            {storeLocation && (
              <div className="rounded-lg overflow-hidden border border-gray-200">
                <InteractiveMap
                  initialLocation={storeMapLocation}
                  height="256px"
                  showSearch={false}
                  showGeolocation={false}
                  markerDraggable={false}
                  showZoneCircles={true}
                  zoneRadii={zones.filter(z => z.is_active).map(z => {
                    // Extract max distance from distance_band (e.g., "5_8" -> 8)
                    const parts = z.distance_band?.split('_');
                    return parts && parts.length === 2 ? Number(parts[1]) : 0;
                  }).filter(r => r > 0)}
                />
                <div className="p-3 bg-gray-50 text-sm text-gray-600">
                  <p className="font-medium">{storeLocation.name || 'Localização da Loja'}</p>
                  <p>{storeLocation.address}, {storeLocation.city} - {storeLocation.state}</p>
                  {storeMapLocation && (
                    <p className="text-xs text-gray-400 mt-1">
                      Coordenadas: {storeMapLocation.latitude.toFixed(6)}, {storeMapLocation.longitude.toFixed(6)}
                    </p>
                  )}
                </div>
              </div>
