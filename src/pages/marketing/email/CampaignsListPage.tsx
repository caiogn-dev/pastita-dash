  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-row max-sm:flex-col sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-gray-900 dark:text-white">Campanhas de Email</h1>
          <p className="text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">{campaigns.length} campanha(s)</p>
        </div>
      ...
      {campaigns.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)] mb-6">Crie sua primeira campanha de email marketing</p>
          <Button onClick={() => navigate('/marketing/email/new')}>
            <PlusIcon className="w-5 h-5 mr-2" />
            Criar Campanha
          </Button>
              ...
              <Card key={campaign.id} className="p-4 hover:shadow-md transition-shadow">
                <div className="flex flex-row max-lg:flex-col lg:items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)] truncate mb-2">{campaign.subject}</p>
                  ...
                  {campaign.status === 'sent' && (
                    <div className="flex items-center gap-6 px-4 py-2 bg-gray-50 dark:bg-[var(--dark-bg-card,#1a1a1a)] rounded-lg">
                      <div className="text-center">
                        <p className="text-lg font-bold text-gray-900 dark:text-white">{campaign.emails_sent}</p>
                        <p className="text-xs text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">Enviados</p>
                      </div>
                      <div className="text-center">
                        <p className="text-lg font-bold text-blue-600">{getOpenRate(campaign)}%</p>
                        <p className="text-xs text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">Abertura</p>
                      </div>
                      <div className="text-center">
                        <p className="text-lg font-bold text-green-600">{getClickRate(campaign)}%</p>
                        <p className="text-xs text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">Cliques</p>
                      </div>
                    </div>
                  )}
      ...
      >
        {loadingRecipients ? (
          <div className="py-12 text-center">
            ...
            <p className="text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">Carregando destinatários...</p>
          </div>
        ) : recipients.length === 0 ? (
          <div className="py-12 text-center">
            <EnvelopeIcon className="w-12 h-12 text-gray-300 mx-auto mb-2" />
            <p className="text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">Nenhum destinatário encontrado</p>
          </div>
        ) : (
          <div className="space-y-4">
            {/* Summary */}
            <div className="grid grid-cols-4 gap-4 p-4 bg-gray-50 dark:bg-[var(--dark-bg-card,#1a1a1a)] rounded-lg">
              <div className="text-center">
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{recipients.length}</p>
                <p className="text-xs text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">Total</p>
              </div>
              <div className="text-center">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">Abertos</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-red-600">
                  {recipients.filter(r => r.status === 'failed' || r.status === 'bounced').length}
                </p>
                <p className="text-xs text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">Falhas</p>
              </div>
            </div>
            ...
            <div className="max-h-96 overflow-y-auto">
              <table className="w-full">
                <thead className="bg-gray-50 dark:bg-[var(--dark-bg-card,#1a1a1a)] sticky top-0">
                  <tr>
                    <th className="text-left text-xs font-medium text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)] px-4 py-2">Email</th>
                    <th className="text-left text-xs font-medium text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)] px-4 py-2">Nome</th>
                    <th className="text-left text-xs font-medium text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)] px-4 py-2">Status</th>
                    <th className="text-left text-xs font-medium text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)] px-4 py-2">Enviado em</th>
                    <th className="text-left text-xs font-medium text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)] px-4 py-2">Erro</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {recipients.map((recipient) => (
                    <tr key={recipient.id} className="hover:bg-gray-50 dark:hover:bg-[var(--dark-bg-hover,#161616)] dark:bg-[var(--dark-bg-card,#1a1a1a)]">
                      <td className="px-4 py-2 text-sm text-gray-900 dark:text-white">{recipient.email}</td>
                      <td className="px-4 py-2 text-sm text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">{recipient.name || '-'}</td>
                      ...
                      <td className="px-4 py-2 text-sm text-gray-500 dark:text-[var(--dark-text-secondary,#a1a1aa)]">
