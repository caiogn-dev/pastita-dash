...
export default function IngredientsPage() {
  ...
  const [search,setSearch]=useState(''); const [category,setCategory]=useState(''); const [aba,setAba]=useState<'loja'|'base'>('loja'); const [totalBase,setTotalBase]=useState(0);
  ...
  const load=useCallback(async()=>{ setLoading(true); try {
    ...
    setItems([...normalizePaginatedResponse<Ingredient>(meus.data),...normalizePaginatedResponse<Ingredient>(base.data)]);
    setTotalBase(base.data?.count??0);
  } catch { toast.error('Não foi possível carregar os ingredientes'); } finally { setLoading(false); }},[storeId,buscaBase]);
  useEffect(()=>{load();},[load]);
  ...
  return <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-5">
    ...
    {aba==='base'&&totalBase>daBase.length&&<p className="text-xs text-amber-700">Mostrando {daBase.length} de {totalBase} alimentos oficiais — refine a busca para chegar no que falta.</p>}
