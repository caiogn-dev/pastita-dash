...
export default function RecipeBuilder({storeSlug,ingredients,productId,storeUuid}:{storeSlug?:string;ingredients:Ingredient[];productId?:string;storeUuid?:string}) {
  ...
  return <Card className="p-5 space-y-4">{!embutido&&<div><h2 className="text-lg font-semibold">Receita do prato</h2><p className="text-sm opacity-65">Escolha o prato e informe os ingredientes e seus pesos. O resultado vira a tabela nutricional dele.</p></div>}<div className="grid md:grid-cols-3 gap-3">{!embutido&&<label className="text-sm md:col-span-1">Produto<select className="mt-1 w-full rounded border p-2 bg-transparent" value={product} onChange={e=>setProduct(e.target.value)}><option value="">Selecione…</option>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>}<label className="text-sm">Porção (g)<input type="number" min="1" className="mt-1 w-full rounded border p-2 bg-transparent" value={serving} onChange={e=>setServing(e.target.value)}/></label><label className="text-sm">Medida caseira<input className="mt-1 w-full rounded border p-2 bg-transparent" value={measure} onChange={e=>setMeasure(e.target.value)}/></label></div>{product&&<><div className="rounded-lg border border-black/10 p-3 space-y-2"><div className="flex flex-wrap items-baseline justify-between gap-2 text-sm"><span className="opacity-70">Composição</span><span className="tabular-nums"><b>{pesoTotal||0} g</b> no total<span className="opacity-60"> · porção {serving} g</span></span></div><BarraDeComposicao fatias={fatias}/></div>
      <div className="space-y-2">{rows.map((row,index)=><div key={`${row.ingredient}-${index}`} className="grid grid-cols-[1fr,120px,auto] gap-2"><SeletorDeIngrediente storeId={storeUuid} valor={row.ingredient?{id:row.ingredient,display_name:nomes[row.ingredient]||ingredients.find(i=>i.id===row.ingredient)?.display_name||'Ingrediente'}:null} aoEscolher={op=>{setNomes(n=>({...n,[op.id]:op.display_name}));setRows(rows.map((r,i)=>i===index?{...r,ingredient:op.id}:r))}}/><div className="relative"><input aria-label="Quantidade em gramas" type="number" min="0.001" step="0.1" className="w-full rounded border p-2 pr-12 bg-transparent tabular-nums" value={row.quantity_g} onChange={e=>setRows(rows.map((r,i)=>i===index?{...r,quantity_g:e.target.value}:r))}/><span className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-xs opacity-60 tabular-nums">{pesoTotal>0&&Number(row.quantity_g)>0?`${Math.round(Number(row.quantity_g)/pesoTotal*100)}%`:'g'}</span></div><button className="px-3 text-red-600" onClick={()=>setRows(rows.filter((_,i)=>i!==index))}>Remover</button></div>)}</div><div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={()=>setRows([...rows,{ingredient:'',quantity_g:'100'}])}>Adicionar ingrediente</Button><Button disabled={saving||!rows.length} onClick={save}>{saving?'Calculando…':'Salvar e calcular'}</Button></div>{calc&&(()=>{const c=calc;const pronto=!c.incomplete_ingredients?.length&&Boolean(c.front_of_pack?.conclusivo)&&Boolean(c.allergens?.revisado);return <div className="rounded-lg border border-black/10 p-3 space-y-3">
