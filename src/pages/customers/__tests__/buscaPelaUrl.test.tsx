...
describe('busca de clientes pela URL', () => {
  ...
  it('monta o link do relatório pelo TELEFONE, não pelo nome', () => {
    ...
    expect(urlDeClienteBuscado({ phone: '5563992618115', name: 'Leani' }))
      .toBe('/customers?busca=5563992618115');
  });

  it('cai no nome quando não há telefone — melhor filtrar mal que não filtrar', () => {
    expect(urlDeClienteBuscado({ phone: '', name: 'Leani Rodrigues' }))
      .toBe('/customers?busca=Leani+Rodrigues');
  });

  it('sem telefone e sem nome, não inventa link', () => {
    expect(urlDeClienteBuscado({ phone: '', name: '' })).toBeNull();
  });
});
