...
export default function InstagramInbox() {
  ...
  const loadAccounts = async () => {
    try {
      ...
      if (response.results?.length > 0) {
        setSelectedAccountId(response.results[0].id);
      }
    } catch (err) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await instagramService.sendTyping(selectedAccountId, selectedConversation.participant_id, true);
      
      // Send message
      const message = await instagramService.sendMessage({
        account_id: selectedAccountId,
      ...
      await instagramService.sendTyping(selectedAccountId, selectedConversation.participant_id, false);
    } catch (err) {
