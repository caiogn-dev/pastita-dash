...
export const usePaymentsWebSocket = () => {
  const { token, isAuthenticated } = useAuthStore();
  const handlersRef = useRef<Map<string, MessageHandler>>(new Map());

  useEffect(() => {
    if (isAuthenticated && token) {
      paymentsWS.connect(getWebSocketUrl('/ws/payments/'), token);
    }
  ...
  const subscribe = useCallback((event: string, handler: MessageHandler) => {
    handlersRef.current.set(event, handler);
    return paymentsWS.on(event, handler);
  }, []);

  const unsubscribe = useCallback((event: string) => {
    const handler = handlersRef.current.get(event);
    if (handler) {
      paymentsWS.off(event, handler);
      handlersRef.current.delete(event);
    }
  }, []);

  const subscribeToPayment = useCallback((paymentId: string) => {
    paymentsWS.send({ type: 'subscribe_payment', payment_id: paymentId });
  }, []);
  ...
  return {
    ...
    unsubscribe,
    subscribeToPayment,
    isConnected: paymentsWS.isConnected(),
  };
};
...
export const useOrdersWebSocket = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && token) {
      ordersWS.connect(getWebSocketUrl('/ws/orders/'), token);
    }
  ...
  const subscribe = useCallback((event: string, handler: MessageHandler) => {
    handlersRef.current.set(event, handler);
    return ordersWS.on(event, handler);
  }, []);

  const unsubscribe = useCallback((event: string) => {
    const handler = handlersRef.current.get(event);
    if (handler) {
      ordersWS.off(event, handler);
      handlersRef.current.delete(event);
    }
  }, []);

  const subscribeToOrder = useCallback((orderId: string) => {
    ordersWS.send({ type: 'subscribe_order', order_id: orderId });
  }, []);
  ...
  return {
    ...
    unsubscribe,
    subscribeToOrder,
    isConnected: ordersWS.isConnected(),
  };
};
