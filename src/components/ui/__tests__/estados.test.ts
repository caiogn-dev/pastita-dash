...
describe('estadoDePedido', () => {
  it('traduz e dá o tom pelo significado, não pela tela', () => {
    ...
    expect(estadoDePedido('preparing')).toEqual({ rotulo: 'Preparando', tone: 'info' });
    expect(estadoDePedido('out_for_delivery')).toEqual({ rotulo: 'Em entrega', tone: 'info' });
    expect(estadoDePedido('delivered')).toEqual({ rotulo: 'Entregue', tone: 'success' });
    expect(estadoDePedido('completed')).toEqual({ rotulo: 'Concluído', tone: 'success' });
